...
LLMs' web interfaces.
%  e.g., ChatGPT admits ``ChatGPT can make mistakes. Check important info''.
In the face of such unreliability, the results from LLMs used in mission-critical applications 
...
A key step in this verification pipeline is faithful translation of the natural language to
a logical form.
This is difficult due to the ambiguous nature of natural language statements, especially
informally expressed statements.
Work in this area includes LINC~\cite{OG+23}, FOLIO~\cite{HS+24}, and LINA~\cite{LL+24-arXiv}.
This paper makes another contribution, taking a new iterative approach to the translation
process, zigzagging between natural language and logic until convergence is achieved.
% (hence the 'ZZ' in the tool name).
...
Figure~\ref{ZigZagLoop} shows the overall process of ShZZaM.
Initially LLM-N is used to normalize the text, breaking down long sentences, converting metaphors 
to their literal meaning, etc.
Each of the following ``ZigZag''s uses a combination of LLMs and ATP tools to translate natural 
language to logic and back to natural language.
LLM-L+ATP is used to translate the natural language to typed first-order logic in the 
TFF syntax~\cite{BP13-TFF1} of the TPTP World~\cite{Sut24}.
LLM-NL is used to translate the logic back to natural language.
LLM-S is then used to judge the similarity in meaning of the new and previous natural language
statements.
...
% The result from the ATP system is reported in the SZS format~\cite{Sut08-KEAPPA}.
If the similarity between the final natural language and the original natural language
% (which is computed in the zigzag step - see below) 
is above the ``zigzagging sequence threshold'' the translation is complete.
...
\begin{figure*}[bt]
\centering
\includegraphics[width=\textwidth]{ZigZagLoop.pdf}
\alt{The zigzag sequence of natural language to logic to natural language translation.}
...
LLMs and ATP tools.
Figure~\ref{ZigLoop} shows the details.
LLM-L is used to translate from natural language to logic.
The translation is successively checked by ATP tools for syntax errors and type errors.
If an error occurs the error message is captured and passed back into the LLM-L for another 
attempt.
When syntactically and type correct logic is created, LLM-NL translates the logic back to
...
\begin{figure*}[bt]
\centering
\includegraphics[width=\textwidth]{ZigLoop.pdf}
\alt{The details of the translation from natural language to logic.}
\caption{Details of one zigzag}
\label{ZigLoop}
\end{figure*}

LLMs can translate between natural language and TPTP format TFF logic because they are exposed to
enough of each in their training.
...
Proofs~\cite{BH+15}, and exports of the Mizar Mathematical Library~\cite{Urb03}, etc.

%--------------------------------------------------------------------------------------------------
\section{Implementation and Testing}

ShZZaM is implemented in Python.
The LLMs available are ChatGPT, Gemini, Claude, and DeepSeek.
Access to ATP is provided via the TPTP World's SystemOnTPTP service~\cite{Sut00-CADE-17}.
ShZZaM has parameters that allow the selection of LLMs (default ChatGPT) and ATP systems (default
...
\smalltt{https://github.com/TPTPWorld/ShZZaM}.
It can also be run in default mode in SystemB4TPTP, at 
\smalltt{https://tptp.org/cgi-bin/SystemB4TPTP}.
% by selecting ``English'' as the ``Input as in'' option.
An API key has to be provided in an initial comment line, e.g.,
\begin{verbatim}
# OPENAI_API_KEY=the_user_api_key
Show me that Socrates was mortal.
\end{verbatim}
% DISCUSS ``Is there'' and ``Show that''.
% No tool needs to be selected -- just ``ProcessProblem''.

\vspace*{-1em}
Initial testing has been done on 12 test texts from SUMO-based research~\cite{NP01,TP+25-NeSy}.
...
Show that Sweden will protect Germany.''.
...
Show that the withdrawal caused penalty.''.
An example of the third type is:
``The tornado damaged the house.
The tornado did not damage the house.''.
Testing used the default settings, except that Gemini was used for the language translations.
...
ChatGPT produced the best results, with 32 of its 36 results confirmed by the ATP system,
while Gemini achieved 24 out of 36.
In two examples Gemini used typed extended first-order logic~\cite{SK18}, i.e., with boolean terms.
The stochastic variations are interesting:
Of the 24 sets of three test runs, 10 ChatGPT sets and 7 Gemini sets were confirmed by the ATP
system in all three runs.
Manual inspection of the logic outputs showed that only 6 of the ChatGPT sets and 1 of the Gemini
...
Future work includes adding access to more LLMs, e.g., Grok, testing over larger datasets, e.g., 
the FOLIO~\cite{HS+24} and ProofWriter~\cite{TDC21} datasets.
The main weakness of ShZZaM (and speculatively all other LLM-based natural language to logic 
translators) is its stochastic nature -- the logic produced typically varies between runs.
